...
@Component({
  selector: 'app-versio-history',
  templateUrl: './versio-history.component.html',
  styleUrls: ['./versio-history.component.scss']
})
export class VersioHistoryComponent {
  ...
  PageSize: number = 10;
  totalRecords: number;
  displayedColumns: string[] = ['Column Name', 'Old Value', 'New Value', 'Modified By', 'Modified Date'];
  ...
  getVesrionHistory() {
    let data = {
      Page: { PageNumber: this.PageNumber, PageSize: this.PageSize, Filter: { Id: this.clientId, ModuleName: this.moduleName } }
    };

    this.spinner.show();
    this.clientService.getVersionHistory(data).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
