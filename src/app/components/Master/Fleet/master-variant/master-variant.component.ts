...
@Component({
  ...
  styleUrls: ['./master-variant.component.scss']
})

export class MasterVariantComponent {
  mainThemeClass = '';
  localTheme: any;
  currentUser: any;
  theme = {
  ...
  displayedColumns: string[] = ['Variant Name', 'JATO ID', 'Model Name', 'Engine CC', 'Seat Capacity', 'Emission Level', 'Rv Deviation', 'Rmt Deviation', 'Maint. Sch.', 'Fuel Type', 'Dimension', 'Warranty Year', 'Warranty Km', 'Fuel Capacity', 'Tyre Size', 'Approved By', 'Created By', 'Modified By', 'WFStatus', 'Action'];
  ...
  getAllVariants() {
    const data =
    {
      Page:
      {
        ...
        Filter: {
          ...
          // VariantName: this.selectedFilter.filterName == "Variant Name" ? this.searchText : "",
          // ModelName: this.selectedFilter.filterName == "Model Name" ? this.searchText : "",
          // JatoId: this.selectedFilter.filterName == "Jato Id" ? Number(this.searchText) : 0,
          // FuelType: this.selectedFilter.filterName == "Fuel Type" ? this.searchText : "",
        }
      }
    }
    ...
    this.spinner.show();
    this.clientService.getAllVariants(formData).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
          this.VariantsList = res.Records;
          this.totalRecords = res.TotalCount;
          this.selectedRowIndex = null;
          this.VariantsList.map((item, index) => {
            item.position = index;
          });
          this.spinner.hide();
        }
  ...
  addClick() {
    this.router.navigate(["/add-variant"], {
  ...
  copyVariant(element: any) {
    ...
    this.spinner.show();
    this.clientService.copyVariant(data).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
          this.toaster.success('Variant Copied Successfully', undefined, {
            positionClass: 'toast-top-center'
          });
          this.spinner.hide();
          this.getAllVariants();
  ...
  exportClick() {
    this.spinner.show();
    this.clientService.exportVariantExcel().subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
          this.commonService.convertBase64ToExcel(res.ExcelData, 'Variant');
          this.spinner.hide();
        }
        else {
          this.toaster.error(res?.Errors[0]?.Message, undefined, {
