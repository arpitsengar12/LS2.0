...
@Component({
  selector: 'app-contact-agreement',
  templateUrl: './contact-agreement.component.html',
  styleUrls: ['./contact-agreement.component.scss']
})
export class ContactAgreementComponent {
  mainThemeClass = '';
  localTheme: any;
  currentUser: any;

  @ViewChild('contractAgTableList') contractAgTableList!: MatTable<any>;
  sortState: string = '';
  sortDirection: string = '';

  displayedColumns: string[] = ['Prospect Name', 'Agreement No', 'Agreement Date', 'Lease Type', 'Vehicle Type', 'Approved', 'Is Online', 'Is Admin', 'WFStatus', 'Action'];
  ContractAgList: any[] = [];
  totalRecords: any;
  PageNumber: number = 1;
  PageSize: number = 10;
  ...
  filterList = [
    { filterName: 'Prospect Name' },
    { filterName: 'Agreement No' },
    { filterName: 'Lease Type' },
    { filterName: 'Vehicle Type' },
    { filterName: 'Is Approved' },
  ...
  GetAllContractAgreement() {
    const data =
    {
      Page:
      {
        ...
        Filter: {
          ...
          // ProspectName: this.selectedFilter.filterName == "Prospect Name" ? this.searchText : "",
          // AgreementNo: this.selectedFilter.filterName == "Agreement No" ? this.searchText : "",
          // LeaseType: this.selectedFilter.filterName == "Lease Type" ? this.searchText : "",
          // VehicleType: this.selectedFilter.filterName == "Vehicle Type" ? this.searchText : "",
          // Approved: this.selectedFilter.filterName == "Is Approved" ? (this.searchText == 'Yes' || this.searchText == 'yes' ? true : false) : null, // null
          // IsOnline: this.selectedFilter.filterName == "Is Online" ? (this.searchText == 'Yes' || this.searchText == 'yes' ? true : false) : null, // null
          // IsAdmin: this.selectedFilter.filterName == "Is Admin" ? (this.searchText == 'Yes' || this.searchText == 'yes' ? true : false) : null, // null
        }
      }
    }
    ...
    this.spinner.show();
    this.clientService.GetAllContractAgreement(formData).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
          this.ContractAgList = res.Records;
          this.totalRecords = res.TotalCount;
          this.selectedRowIndex = null;
          this.ContractAgList.map((item, index) => {
            item.position = index;
          });
          this.spinner.hide();
        }
  ...
  announceSortChange(sortState: Sort) {
    if (sortState.direction) {
      ...
      this.ContractAgList = this.commonService.customSort(this.ContractAgList, sortState.active, sortState.direction);
      this.contractAgTableList.renderRows();
      console.log(this.ContractAgList);
  ...
  addClick() {
    this.router.navigate(["/create-contract-agreement"], {
