...
@Component({
  selector: 'app-master-states',
  templateUrl: './master-states.component.html',
  styleUrls: ['./master-states.component.scss']
})
export class MasterStatesComponent {
  ...
  @ViewChild('stateTableList') stateTableList!: MatTable<any>;
  sortState: string = '';
  sortDirection: string = '';

  displayedColumns: string[] = ['Region Name', 'State Name', 'VAT Account No.', 'Old State Code', 'Last lnv', 'Ora Vat Per', 'Vat Per Fin', 'WFStatus', 'Action'];
  ...
  filterList = [
    ...
    { filterName: 'VAT Acc No' },
    { filterName: 'Old State Code' },
    { filterName: 'Last lnv' },
    { filterName: 'Ora Vat Per' },
    { filterName: 'Ora Vat Fin' },
  ];
  ...
  constructor(
    ...
    if (history.state.level) {
      ...
      this.workFlowEnabled = history.state.workFlowEnabled;
    ...
    //   this.displayedColumns = ['Region Name', 'State Name', 'VAT Account No.', 'Old State Code', 'Last lnv', 'Ora Vat Per', 'Vat Per Fin', 'WFStatus', 'Action'];
    // }
  }
  ...
  getAllStates() {
    const data =
    {
      Page:
      {
        ...
        Filter: {
          ...
          // StateName: this.selectedFilter.filterName == "State Name" ? this.searchText : "",
          // VatAccountNo: this.selectedFilter.filterName == "VAT Acc No" ? this.searchText : "",
          // OldStateCode: this.selectedFilter.filterName == "Old State Code" ? this.searchText : "",
          // LastInvNo: this.selectedFilter.filterName == "Last lnv" ? this.searchText : "",
          // OraVatPer: this.selectedFilter.filterName == "Ora Vat Per" ? this.searchText : "",
          // VatPerFin: this.selectedFilter.filterName == "Ora Vat Fin" ? this.searchText : "",
        }
      }
    }
    ...
    this.spinner.show();
    this.clientService.getAllStates(formData).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
          this.StatesList = res.Records;
          this.totalRecords = res.TotalCount;
          this.selectedRowIndex = null;
          this.StatesList.map((item, index) => {
            item.position = index;
          });
          this.spinner.hide();
        }
  ...
  announceSortChange(sortState: Sort) {
    if (sortState.direction) {
      ...
      this.StatesList = this.commonService.customSort(this.StatesList, sortState.active, sortState.direction);
      this.stateTableList.renderRows();
    }
  }
  ...
  openEditModal(event: any) {
    this.router.navigate(["/add-state"], {
      state: {
        level: this.AccessLevel, id: event.StateID, mode: "edit", IsAuditTrail: this.IsAuditTrail
      }
    });
  }
  ...
  openViewDialog(event: any) {
    this.router.navigate(["/add-state"], {
      state: {
        level: this.AccessLevel, id: event.StateID, mode: "view", IsAuditTrail: this.IsAuditTrail
      }
    });
