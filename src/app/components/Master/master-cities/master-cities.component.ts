...
@Component({
  ...
  styleUrls: ['./master-cities.component.scss']
})
...
export class MasterCitiesComponent {
  ...
  @ViewChild('cityTableList') cityTableList!: MatTable<any>;
  sortState: string = '';
  sortDirection: string = '';

  displayedColumns: string[] = ['City Name', 'RV Deviation', 'RMT Deviation', 'Ins Zone Private', 'Ins Zone Taxi', 'WFStatus', 'Action'];
  ...
  getAllCities() {
    ...
    this.spinner.show();
    this.clientService.getAllCities(formData).subscribe((res: any) => {
      if (res) {
        if (!res.HasErrors) {
  ...
  announceSortChange(sortState: Sort) {
    if (sortState.direction) {
      ...
      this.CityList = this.commonService.customSort(this.CityList, sortState.active, sortState.direction);
      this.cityTableList.renderRows();
    }
  }
  ...
  openEditModal(event: any) {
    this.router.navigate(["/add-city"], {
      state: {
        level: this.AccessLevel, id: event.CityID, mode: "edit", IsAuditTrail: this.IsAuditTrail
      }
    });
  }
  ...
  openViewDialog(event: any) {
    this.router.navigate(["/add-city"], {
      state: {
        level: this.AccessLevel, id: event.CityID, mode: "view", IsAuditTrail: this.IsAuditTrail
      }
    });
